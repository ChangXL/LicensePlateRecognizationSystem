...
if __name__ == '__main__':
    cur_dir = sys.path[0]
    data_dir = os.path.join(cur_dir, 'dataset_ch/train')
    test_dir = os.path.join(cur_dir, 'dataset_ch/test')
    train_model_path = os.path.join(cur_dir, './model_ch/model.ckpt')
    model_path = os.path.join(cur_dir,'./model_ch/model.ckpt-530')
    ...
    if train_flag == 0:
        ...
        t2=cv2.getTickCount()
        costtime=(t2-t1)/cv2.getTickFrequency()/60
        print(costtime)
    else:
        # 测试部分
